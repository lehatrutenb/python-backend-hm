...
class TestPostsApi(APITestCase):
    def setUp(self):
        ...
        self.mark1 = MarkPost.objects.create(author=self.user1, post=self.post1, value=3)
        self.mark2 = MarkPost.objects.create(author=self.user2, post=self.post1, value=-2)

    def test_mark_post_get(self):
        url = '/api/messanger/mark_posts/{}/'.format(self.mark1.id)
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.json()["id"], self.mark1.id)

    def test_mark_post_post(self):
        url = '/api/messanger/mark_posts/'
        data = {
